"""pi/scripts/simulate.py – Generate synthetic SensorPacket streams for integration testing.

Produces one of five named scenarios as newline-delimited JSON on stdout
(or socket / file) so the full edge→PINN loop can be exercised without
real hardware.

Usage::

    python -m pi.scripts.simulate \\
        ...
        [--host <str>] [--port-out <int>]      # for output-mode=socket
        [--checkpoint <path>]        # spawn orchestrator_bridge and pipe packets through it
"""
...
from pi.orchestrator_bridge import OrchestratorBridge
from pi.packet_builder import build_sensor_packet
...
class _PrintingActuator:
    """Actuator that prints received OrchestrationCommands as JSON lines on stdout."""

    def apply(self, command: dict[str, Any]) -> bool:
        """Print *command* as a JSON line on stdout."""
        sys.stdout.write(json.dumps(command, separators=(",", ":")) + "\n")
        sys.stdout.flush()
        return True


def _run_with_checkpoint(packets: Iterator[dict], args: argparse.Namespace) -> int:
    """Pipe generated packets through the PINN orchestrator_bridge."""
    pinn_root = os.environ.get("AURA_EDGE_PINN_ROOT")
    if not pinn_root:
        sys.stderr.write(
            "simulate: --checkpoint requires the AURA_EDGE_PINN_ROOT "
            "environment variable to point at a PINN-AURA-MFP clone\n"
        )
        return 1
    bridge = OrchestratorBridge(
        ...
        actuator=_PrintingActuator(),
    )
    with bridge:
        for pkt in packets:
            if _INTERRUPTED:
                break
            bridge.send_packet(pkt)
            if args.cadence > 0.0:
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p.add_argument(
        "--output-mode", choices=["stdout", "socket", "file"], default="stdout",
    )
    p.add_argument(
        "--output", default=None,
    ...
    if args.checkpoint is not None:
        return _run_with_checkpoint(packets, args)
