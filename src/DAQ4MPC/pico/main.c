 ...
 *   4. Emit the JSON line over USB CDC (primary link to the workstation)
 *      and UART0 (GP0/GP1, to OpenLog for local black-box logging).
...
int main(void)
{
    ...
    while (1) {
        ...
        /* ── ADC sensors (oversampled, RAW -- no scaling in firmware) ─── */
        uint16_t t_amb_raw, ws_raw;
        adc_sensors_read(&t_amb_raw, &ws_raw);

        /* ── Build raw JSON packet ─────────────────────────────────── */
        int json_len = json_build_packet(
            json_buf, sizeof(json_buf),
            timestamp_str,
            t_s, t_amb_raw, ws_raw,
