...
class ThermocoupleMAX31856(SensorBase):
    """Driver for a bank of MAX31856 SPI thermocouple amplifiers.

    Channel count is driven entirely by ``len(cs_pins)`` -- confirmed
    this session: 5 real thermocouples (not counting the SparkFun
    weather:bit's onboard BME280, which is a separate ambient sensor on
    a different bus). Was previously hardcoded to a 4-channel loop
    regardless of how many cs_pins were actually passed in -- fixed to
    use the real channel count throughout.

    The DEFAULT cs_pins below is still the original 4-pin (5,6,7,8)
    assignment -- NOT updated to 5 pins, since the 5th chip-select GPIO
    hasn't been assigned yet (needs to avoid conflicting with the
    weather:bit's I2C pins and OpenLog's UART pins, already assigned in
    pico/config.h -- a real schematic decision, not something to guess
    here). Pass the real 5-pin tuple explicitly once that's decided:
    ``ThermocoupleMAX31856(spi=..., cs_pins=(5, 6, 7, 8, <pin>))``.
    ...
    # ------------------------------------------------------------------
    def read(self):
        """Return a list of raw TC counts (int32), one per configured channel.

        Length matches len(cs_pins) -- was previously hardcoded to
        always return exactly 4 values regardless of how many channels
        were actually configured, which would have silently truncated
        or misrepresented data for the real 5-channel setup. Fixed to
        use the actual channel count throughout.
