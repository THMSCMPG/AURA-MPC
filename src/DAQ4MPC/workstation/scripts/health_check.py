"""workstation/scripts/health_check.py – Verify serial link and sensor sanity.
...
STALE, FLAGGED NOT REWRITTEN (2026-08-22): both the live-serial path
(_collect/main) and the --mock path (_run_mock_checks) are built around
the old raw-frame binary format (workstation.serial_reader.SerialReader, COBS
framing) and old sensors (pyranometer, RS-485 anemometer, camera) that no
longer exist. This needs a real rewrite against the new raw-packet
JSON-line format (see pico/json_builder.h) and the confirmed sensor set
(T_amb_raw/WS_raw + eventually 5 thermocouples) -- deliberately not done
here, since the workstation-side integration this depends on is itself
being built last, after training and the physical hardware are ready
(see checklist). SerialReader/SkyCameraCapture imports below are function-
-local specifically so this file still IMPORTS cleanly; calling the
functions that need them will raise a clear ImportError pointing at this
note, not a confusing crash on module load.

Usage
-----
    python -m workstation.scripts.health_check [--port /dev/serial0] [--baud 115200] [--count 10]
    python -m workstation.scripts.health_check --input-mode file --input fixtures/sample_frames.bin
    python -m workstation.scripts.health_check --mock         # CI / pre-flight, no hardware
"""
...
def _collect(reader, cal: Calibration, count: int) -> dict:
    """STALE -- see module docstring. Calling this raises ImportError
    (no workstation.serial_reader anymore) rather than working against real/mock
    data; needs the rewrite described above before use."""
...
def main(argv=None) -> int:
    ...
    if args.input_mode == "file":
        ...
        from workstation.serial_reader import SerialReader  # noqa: PLC0415 -- see module docstring, this raises ImportError until rewritten
...
def _run_mock_checks() -> int:  # noqa: C901 – linear sequence, readability wins.
    from workstation.calibration import CalibrationLoader
    from workstation.camera import SkyCameraCapture  # noqa: PLC0415 -- see module docstring, raises ImportError until rewritten
    from workstation.packet_builder import build_sensor_packet, validate_sensor_packet
    from workstation.serial_reader import SerialReader  # noqa: PLC0415 -- ditto
