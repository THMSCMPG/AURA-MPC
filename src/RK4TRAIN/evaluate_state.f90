...
contains
    ...
    subroutine compute_G_eff()
        ...
        ! BUGFIX (identical issue found/fixed in main.f90 during the Section 1
        ! sprint): flooring sin_elev to 0 here, then using the floored value
        ! in the AOI formula below, let a tilted panel pick up spurious
        ! nonzero G_eff from pure azimuth geometry even when the sun is truly
        ! below the horizon. Check the TRUE (unclamped) value and zero G_eff
        ! outright instead.
