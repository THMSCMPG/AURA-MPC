...
def test_pinn_loading():
    ...
    try:
        sys.path.insert(0, str(Path(__file__).parent.parent))  # .../RK4TRAIN/ml
        from models import PINNSurrogate

        model = PINNSurrogate(input_dim=19, hidden_dim=128, num_residual_blocks=4, num_outputs=4)
        print(f"✓ Model created: {model.__class__.__name__}")
        print(f"  Parameters: {sum(p.numel() for p in model.parameters()):,}")

        # Test forward pass -- forward() returns a dict of 4 separate [B,1]
        # tensors (T_operating, T_operating_sigma, eta, eta_sigma), NOT a
        # single stacked [B,4] tensor. This was the original bug here.
        x = torch.randn(2, 19)
        y = model(x)
        expected_keys = {"T_operating", "T_operating_sigma", "eta", "eta_sigma"}
        assert isinstance(y, dict), f"Expected dict output, got {type(y)}"
        assert set(y.keys()) == expected_keys, f"Expected keys {expected_keys}, got {set(y.keys())}"
        for k, v in y.items():
            assert v.shape == (2, 1), f"Expected {k}.shape == (2,1), got {v.shape}"
        print(f"✓ Forward pass works: input (2, 19) → dict of 4x (2, 1) tensors")
...
def test_rk4_validator():
    ...
    try:
        ...
        from sandbox import RK4TRANValidator

        # Check if binary exists
        binary_path = Path(__file__).parent.parent.parent / "main"  # .../RK4TRAIN/main
...
def test_sandbox_training():
    ...
    try:
        ...
        sys.path.insert(0, str(Path(__file__).parent.parent))  # .../RK4TRAIN/ml
        from sandbox import SandboxTrainer, PanelEnv
...
def test_config_loading():
    ...
    try:
        config_path = Path("src/RK4TRAIN/ml/configs/sandbox.yaml")
