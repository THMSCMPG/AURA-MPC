...
def test_model() -> None:
    ...
    try:
        # Derive input_dim from RK4TRANDataset's field-group constants rather
        # than a hardcoded number, so this test can't silently drift out of
        # sync with the dataset schema again (it previously hardcoded 18,
        # stale after the schema changed to 19 -- 7 weather + 4 panel_state +
        # 3 location + 5 time).
        input_dim = (
            len(RK4TRANDataset.WEATHER_FIELDS)
            + len(RK4TRANDataset.PANEL_STATE_FIELDS)
            + len(RK4TRANDataset.LOCATION_FIELDS)
            + len(RK4TRANDataset.TIME_FIELDS)
        )
        model = PINNSurrogate(
            input_dim=input_dim,
            hidden_dim=128,
...
def main() -> None:
    ...
    csv_dir = Path(__file__).parent.parent.parent / "lattice_batches"
    if csv_dir.exists():
        ...
        print("To test with real data, run the RK4TRAIN generator first (bash make.sh && ./main --smoke)")
