...
_LOCATION_DIM = 3
_PANEL_TEMP_DIM = 1
_BASE_INPUT_DIM = _WEATHER_DIM + _PANEL_DIM + _LOCATION_DIM + _PANEL_TEMP_DIM
...
class PINNValidator:
    ...
    def predict(
        ...
        location: dict[str, float],
        time_components: dict[str, float],
        T_panel_initial: float,
    ) -> dict[str, float]:
        """Run the PINN forward pass.

        Args:
            T_panel_initial: starting panel temperature (K) for the 15-min
                transient prediction (see PINNSurrogate, D10) -- required,
                not optional. The MPC candidate-evaluation loop passes the
                currently measured/estimated panel temperature here for
                every candidate orientation being scored.
        """
        with torch.no_grad():
            sample = {
                ...
                "panel_temp": torch.tensor([float(T_panel_initial)], dtype=torch.float32),
            }
            sample = self._normalize_sample(sample)
            x = torch.cat(
                [sample["weather"], sample["panel_state"], sample["location"], sample["time"], sample["panel_temp"]],
                dim=0,
            ).to(self.device)
        ...
        return {
            ...
            "T_after_15min": float(outputs["T_after_15min"].squeeze().item()),
            "T_after_15min_sigma": float(outputs["T_after_15min_sigma"].squeeze().item()),
            "eta_after_15min": float(outputs["eta_after_15min"].squeeze().item()),
            "eta_after_15min_sigma": float(outputs["eta_after_15min_sigma"].squeeze().item()),
        }
...
class SandboxPINNAgent:
    ...
    def predict(
        ...
        """Run PINN inference (steady-state + 15-min transient) and, if
        requested, cross-validate the STEADY-STATE prediction against
        RK4TRAN's live single-state evaluator.

        NOTE on scope: bias correction (self._bias_correct) only ever
        covers T_operating/eta -- the steady-state fields -- not
        T_after_15min/eta_after_15min. This isn't an oversight: the live
        Fortran evaluator (evaluate_state.f90) only computes steady state,
        not a transient prediction given an arbitrary starting temperature,
        so there's no RK4TRAN ground truth to bias-correct the transient
        prediction against yet (see checklist Section 1.7 -- a live-callable
        transient/optimal-orientation evaluator was flagged as a future
        TODO, not built). The MPC candidate-evaluation loop (runtime.py)
        uses T_after_15min RAW from pinn_out, uncorrected -- a real,
        honest limitation, not hidden.
        """
        pinn_out = self.pinn.predict(weather, panel_state, location, time_components, T_panel_initial)
